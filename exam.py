...
with st.expander("🏛️ Bölüm Dersleri (MetE)", expanded=True):
    edited_df_dept = st.data_editor(
        ...
        column_config={
            ...
            "Ders Yükü": st.column_config.NumberColumn("Yük", min_value=0, step=1, help="Başlangıç Yükü"),
...
# --- DAĞITIM VE SONUÇLAR ---
st.markdown("---")
# Büyük, ortalanmış buton için kolon ayarı
    ...
    with st.spinner('Algoritma çalışıyor, yükler dengeleniyor...'):
        ...
        if active_dept.empty and active_service.empty:
            ...
            if not parse_error:
                ...
                with tab1:
                    # Plotly ile Havalı Grafik
                    fig = px.bar(
                        df_final, x='name', y='load',
                        text='load',
                        color='load',
                        color_continuous_scale=['#ffcccc', '#E31937'], # Açık kırmızıdan ODTÜ kırmızısına
